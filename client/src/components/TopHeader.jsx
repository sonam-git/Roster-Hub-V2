...
import { faFutbol, faPersonRunning, faCalendarAlt, faPlus, faInbox, faStar } from "@fortawesome/free-solid-svg-icons";
...
export default function TopHeader() {
  ...
  return (
    <div className="w-full flex flex-col sm:flex-row items-center justify-between bg-gray-100 dark:bg-gray-900 py-2 shadow-md mb-4 sticky top-0 z-30 px-4">
      {/* Left: Username and soccer icon */}
      <div className="flex items-center space-x-2 mb-2 sm:mb-0 w-full sm:w-auto justify-center">
      ...
      {/* Center: Menu buttons */}
      <div className="flex flex-row items-center justify-center flex-1 w-full sm:w-auto">
        {BUTTONS.map(btn => (
          <button
            key={btn.key}
            className={`dark:text-gray-900 mx-2 px-4 py-1 rounded-full font-semibold transition-colors text-sm flex items-center gap-2 ${location.pathname === btn.path.split('#')[0] ? "bg-blue-600 text-white" : "bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-blue-900"}`}
            onClick={() => {
