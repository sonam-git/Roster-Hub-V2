...
export default function FormationBoard({ rows, assignments }) {
  return (
    <div className="relative p-4 bg-green-600 dark:bg-gray-800 rounded shadow">
      ...
      <div className="relative w-full h-[500px] mx-auto rounded-lg overflow-hidden bg-green-700 bg-[repeating-linear-gradient(to-bottom,_#006400_0px,_#006400_40px,_#228B22_40px,_#228B22_80px)] border-4 border-white">

        {/* 🟩 Large 3-sided rectangle (no top border) */}
        <div className="absolute top-0 left-[10%] w-[80%] h-[180px] border-l-2 border-r-2 border-b-2 border-white z-10" />
        ...
        <div className="absolute bottom-[-30px] left-1/2 w-[180px] h-[80px] border-2 border-white border-b-0 z-10 transform -translate-x-1/2 rounded-t-full" />
...
function Slot({ slotId, player }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`${base} ${highlight} text-xs text-center`}
    >
      {player ? player.name : "—"}
    </div>
  );
